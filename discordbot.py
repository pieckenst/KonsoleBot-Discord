...
@bot.command()
async def info(ctx):
    embed = discord.Embed(title="<:info:656202310434816011> KonsoleBot", description="Бот пародирующий Терминал Linux и оболочку bash", color=0x00ff00)
...
async def config(ctx):
    ...
    confemb.add_field(name="welcome channel [TextChannel]", value="Выбор канала для отправки приветственных сообщений", inline=False)
    confemb.add_field(name="goodbye channel [TextChannel]", value="Выбор канала для отправки прощальных сообщений", inline=False)
    confemb.add_field(name="sub channel [TextChannel]", value="Выбор канала для отправки предложений", inline=False)
    confemb.add_field(name="ticket channel [TextChannel]", value="Выбор канала для отправки тикетов", inline=False)
...
@bot.group(invoke_without_command=True)
async def sub(ctx):
    subinfo = discord.Embed(title="Команда sub", description="Используется для создания голосования, предложения. ", color=0x00ff00)
    subinfo.add_field(name="Использование", value="``sub faq`` - краткая информация об обозначении реакций.\n ``sub create``- создать предложение. ", inline=True)
...
async def create(ctx,*, arg):
    ...
    submit = discord.Embed(title=f"<:speech_balloon:644199603033473055> Предложение пользователя {user}", description=arg, color=0x00ff00)
    msg = await channel.send(embed=submit)
    emoj = bot.get_emoji(656155011687907358)
    emoj2 = bot.get_emoji(656155011746889758)
...
async def channel(ctx, channel: discord.TextChannel):
    ...
    if author.guild_permissions.manage_messages:
        ...
        await ctx.send("bot: Недостаточно прав для выполнения данной команды")

@sub.command(pass_context=True)
async def faq(ctx):
    subfaq = discord.Embed(title="Информация о реакциях",description="<:green_tick:656155011687907358> - отметка о вашем согласии с предложением.\n <:red_cross:656155011746889758> - отметка о вашем несогласии с предложением. \n <:red_circle:656510683692662795> - предложение отклонено администрацией. ",  color=0x00ff00)
    await ctx.send(embed=subfaq)

initial_extensions = ['cogs.mod'] # Модерация
...
initial_extensions = ['cogs.minigames']
...
initial_extensions = ['cogs.gnulinux']
...
initial_extensions = ['cogs.infosystem']
...
initial_extensions = ['cogs.goodbye']
...
initial_extensions = ['cogs.wallpapers']
...
initial_extensions = ['cogs.system']
